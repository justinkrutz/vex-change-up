...
void set_callbacks() {
  ...
  button_handler.master.up.pressed .set([&](){ score_queue++; score_balls = true; });
  // button_handler.master.up.released .set([&](){ score_balls = false; });
  button_handler.master. x.    pressed .set(rollers_forward);
  button_handler.master. x.    released.set(rollers_stop);
  button_handler.master. b.    pressed .set(rollers_reverse);
